;(function() {
  ...
  function initSalesSection() {
    ...
    if (!window.__deptListDelegated) {
      document.addEventListener('click', function(e){
        ...
        var cardViewBtn = e.target.closest('#deptCardViewBtn');
        if (cardViewBtn) {
          e.preventDefault();
          const cardView = document.getElementById('deptSalesCardView');
          const tableView = document.getElementById('deptSalesTableView');
          if (cardView && tableView) {
            cardView.style.display = 'block';
            tableView.style.display = 'none';
            cardViewBtn.classList.add('active');
            const tableViewBtn = document.getElementById('deptTableViewBtn');
            if (tableViewBtn) tableViewBtn.classList.remove('active');
          }
        }
        var tableViewBtn = e.target.closest('#deptTableViewBtn');
        if (tableViewBtn) {
          e.preventDefault();
          const cardView = document.getElementById('deptSalesCardView');
          const tableView = document.getElementById('deptSalesTableView');
          if (cardView && tableView) {
            ...
            const cardViewBtn = document.getElementById('deptCardViewBtn');
    ...
    // Set up Card View / Table View toggle buttons
    function setupDeptViewToggleButtons() {
      ...
      const deptSalesCardView = document.getElementById('deptSalesCardView');
      const deptSalesTableView = document.getElementById('deptSalesTableView');
      
      if (deptCardViewBtn && deptTableViewBtn && deptSalesCardView && deptSalesTableView) {
        ...
        newCardBtn.addEventListener('click', function(e) {
          e.preventDefault();
          e.stopPropagation();
          deptSalesCardView.style.display = 'block';
          deptSalesTableView.style.display = 'none';
        ...
        newTableBtn.addEventListener('click', function(e) {
          e.preventDefault();
          e.stopPropagation();
          deptSalesCardView.style.display = 'none';
          deptSalesTableView.style.display = 'block';
    ...
    // Set up view toggle buttons immediately and on section load
    setupDeptViewToggleButtons();
  ...
  document.addEventListener('sectionLoaded', function(e){
    if (e && e.detail && e.detail.sectionName === 'sales') {
      setTimeout(function() {
        initSalesSection();
        // Set up view toggle buttons after section loads
        setTimeout(function() {
          const deptCardViewBtn = document.getElementById('deptCardViewBtn');
          const deptTableViewBtn = document.getElementById('deptTableViewBtn');
          const deptSalesCardView = document.getElementById('deptSalesCardView');
  ...
  function loadSubDepartmentsInternal(departmentId, container, apiInstance){
    ...
    const build = function(subDepts){
      ...
      table.innerHTML = '<thead class="table-light"><tr><th>Sub-Department</th><th class="text-end">Gross Sale</th><th class="text-end">Discount Amount</th><th class="text-end">Discount %</th><th class="text-end">Sale Value</th><th class="text-end">Return</th><th class="text-end">GST</th><th class="text-end">Net Sale</th><th class="text-end">Cost</th></tr></thead><tbody id="bulkSubDeptTableBodyTab2"></tbody><tfoot class="table-info fw-bold"><tr id="bulkSubDeptTotalsRowTab2"><td><strong>Total</strong></td><td class="text-end" id="totalGrossSaleTab2">0.00</td><td class="text-end" id="totalDiscountTab2">0.00</td><td class="text-end" id="avgDiscountPercentTab2">0.00%</td><td class="text-end" id="totalSaleValueTab2">0.00</td><td class="text-end" id="totalReturnTab2">0.00</td><td class="text-end" id="totalGSTTab2">0.00</td><td class="text-end" id="totalNetSaleTab2">0.00</td><td class="text-end" id="totalCostTab2">0.00</td></tr></tfoot>';
      ...
      subDepts.forEach(function(sd){
        ...
        const cost = row.querySelector('.bulk-cost-tab2');
        ...
        [gross, disc, ret, gst, cost].forEach(function(inp){ if (inp){ inp.addEventListener('input', recalc); inp.addEventListener('change', recalc); } });
      });
  ...
  function loadDepartmentSummary(sales){ var summarySection = document.getElementById('departmentSummarySection'); var summaryCards = document.getElementById('departmentSummaryCards'); if (!sales || sales.length===0){ if (summarySection) summarySection.style.display='none'; return; } var departmentData = {}; if (appData.departments && appData.departments.length>0){ appData.departments.forEach(function(department){ departmentData[department._id] = { name: department.name, grossSale:0, discount:0, returnAmount:0, gst:0, cost:0, netSale:0, transactions:[] }; }); } var sortedSales = sales.slice().sort(function(a,b){ return new Date(b.date) - new Date(a.date); }); sortedSales.forEach(function(sale){ var departmentId = (sale.departmentId && sale.departmentId._id) || sale.departmentId; if (departmentId){ if (!departmentData[departmentId]){ var deptName = (sale.departmentId && sale.departmentId.name) || 'Unknown Department'; departmentData[departmentId] = { name: deptName, grossSale:0, discount:0, returnAmount:0, gst:0, cost:0, netSale:0, transactions:[] }; } departmentData[departmentId].grossSale += sale.grossSale || 0; departmentData[departmentId].discount += sale.discountAmount || 0; departmentData[departmentId].returnAmount += sale.returnAmount || 0; departmentData[departmentId].gst += sale.gst || 0; departmentData[departmentId].cost += sale.cost || 0; departmentData[departmentId].netSale += sale.netSale || 0; departmentData[departmentId].transactions.push({ date: sale.date, grossSale: sale.grossSale || 0, discount: sale.discountAmount || 0, returnAmount: sale.returnAmount || 0, gst: sale.gst || 0, cost: sale.cost || 0, netSale: sale.netSale || 0, branch: (sale.branchId && sale.branchId.name) || 'Unknown', subDepartment: (sale.subDepartmentId && sale.subDepartmentId.name) || 'Unknown' }); } }); var hasData = Object.values(departmentData).some(function(dept){ return dept.netSale > 0; }); if (summarySection) summarySection.style.display = hasData ? 'block' : 'none'; if (!hasData || !summaryCards) return; summaryCards.innerHTML=''; var departmentsWithSales = Object.values(departmentData).filter(function(dept){ return dept.netSale > 0; }).sort(function(a,b){ return b.netSale - a.netSale; }); var grandTotalGrossSale=0, grandTotalDiscount=0, grandTotalReturn=0, grandTotalGST=0, grandTotalCost=0, grandTotalNetSale=0, grandTotalTransactions=0; departmentsWithSales.forEach(function(department){ grandTotalGrossSale += department.grossSale; grandTotalDiscount += department.discount; grandTotalReturn += department.returnAmount; grandTotalGST += department.gst; grandTotalCost += department.cost; grandTotalNetSale += department.netSale; grandTotalTransactions += department.transactions.length; });
    ...
    if (departmentsWithSales.length > 0) {
      ...
      grandTotalCard.innerHTML = '<div class="card border-0 shadow-lg" style="border: 2px solid #ffc107 !important;"><div class="card-header bg-warning" style="padding: 0.5rem 1rem;"><div class="row align-items-center"><div class="col-md-8"><h5 class="mb-0 fw-bold text-white"><i class="fas fa-calculator me-2"></i>GRAND TOTAL (All Departments)</h5></div><div class="col-md-4 text-end"><span class="badge bg-dark text-white">' + grandTotalTransactions + ' Total Transactions</span></div></div></div><div class="card-body" style="background-color: #fff9e6; padding: 0.75rem 1rem;"><div class="row mb-3"><div class="col-md-2 col-lg-2"><div class="stat-box"><div class="stat-label fw-bold">Gross Sale</div><div class="stat-value fw-bold" style="font-size: 1.2em;">' + grandTotalGrossSale.toLocaleString() + '</div></div></div><div class="col-md-2 col-lg-2"><div class="stat-box"><div class="stat-label fw-bold">Discount</div><div class="stat-value fw-bold" style="font-size: 1.2em;">' + grandTotalDiscount.toLocaleString() + '</div></div></div><div class="col-md-2 col-lg-2"><div class="stat-box"><div class="stat-label fw-bold">Return</div><div class="stat-value fw-bold" style="font-size: 1.2em;">' + grandTotalReturn.toLocaleString() + '</div></div></div><div class="col-md-2 col-lg-2"><div class="stat-box"><div class="stat-label fw-bold">GST</div><div class="stat-value fw-bold" style="font-size: 1.2em;">' + grandTotalGST.toLocaleString() + '</div></div></div><div class="col-md-2 col-lg-2"><div class="stat-box"><div class="stat-label fw-bold">Cost</div><div class="stat-value fw-bold" style="font-size: 1.2em;">' + grandTotalCost.toLocaleString() + '</div></div></div><div class="col-md-2 col-lg-2"><div class="stat-box"><div class="stat-label fw-bold">Net Sale</div><div class="stat-value fw-bold text-success" style="font-size: 1.3em;"><strong>' + grandTotalNetSale.toLocaleString() + '</strong></div></div></div></div></div></div>';
      summaryCards.appendChild(grandTotalCard);
    }
    
    departmentsWithSales.forEach(function(department){ var card=document.createElement('div'); card.className='col-12 mb-4'; card.innerHTML = '<div class="card border-0 shadow-sm h-100"><div class="card-header bg-primary text-white"><div class="row align-items-center"><div class="col-md-8"><h6 class="mb-0"><i class="fas fa-building me-2"></i>' + department.name + '</h6></div><div class="col-md-4 text-end"><span class="badge bg-light text-dark">' + department.transactions.length + ' Transactions</span></div></div></div><div class="card-body"><div class="row mb-3 border-bottom pb-3"><div class="col-md-2 col-lg-2"><div class="stat-box"><div class="stat-label">Gross Sale</div><div class="stat-value">' + (department.grossSale.toLocaleString()) + '</div></div></div><div class="col-md-2 col-lg-2"><div class="stat-box"><div class="stat-label">Discount</div><div class="stat-value">' + (department.discount.toLocaleString()) + '</div></div></div><div class="col-md-2 col-lg-2"><div class="stat-box"><div class="stat-label">Return</div><div class="stat-value">' + (department.returnAmount.toLocaleString()) + '</div></div></div><div class="col-md-2 col-lg-2"><div class="stat-box"><div class="stat-label">GST</div><div class="stat-value">' + (department.gst.toLocaleString()) + '</div></div></div><div class="col-md-2 col-lg-2"><div class="stat-box"><div class="stat-label">Cost</div><div class="stat-value">' + ((department.cost||0).toLocaleString()) + '</div></div></div><div class="col-md-2 col-lg-2"><div class="stat-box"><div class="stat-label">Net Sale</div><div class="stat-value text-success"><strong>' + (department.netSale.toLocaleString()) + '</strong></div></div></div></div><div class="table-responsive" style="overflow-x: auto; -webkit-overflow-scrolling: touch;"><table class="table table-sm table-hover" style="width: 100%; table-layout: fixed; border-collapse: collapse;"><colgroup><col style="width: 10%;"><col style="width: 15%;"><col style="width: 15%;"><col style="width: 10%;"><col style="width: 10%;"><col style="width: 10%;"><col style="width: 10%;"><col style="width: 10%;"><col style="width: 10%;"></colgroup><thead style="display: table-header-group;"><tr><th style="text-align: left; padding: 8px; border: 1px solid #dee2e6; background-color: #f8f9fa; font-weight: 600;">Date</th><th style="text-align: left; padding: 8px; border: 1px solid #dee2e6; background-color: #f8f9fa; font-weight: 600;">Branch</th><th style="text-align: left; padding: 8px; border: 1px solid #dee2e6; background-color: #f8f9fa; font-weight: 600;">Sub-Department</th><th style="text-align: right; padding: 8px; border: 1px solid #dee2e6; background-color: #f8f9fa; font-weight: 600;">Gross</th><th style="text-align: right; padding: 8px; border: 1px solid #dee2e6; background-color: #f8f9fa; font-weight: 600;">Discount</th><th style="text-align: right; padding: 8px; border: 1px solid #dee2e6; background-color: #f8f9fa; font-weight: 600;">Return</th><th style="text-align: right; padding: 8px; border: 1px solid #dee2e6; background-color: #f8f9fa; font-weight: 600;">GST</th><th style="text-align: right; padding: 8px; border: 1px solid #dee2e6; background-color: #f8f9fa; font-weight: 600;">Cost</th><th style="text-align: right; padding: 8px; border: 1px solid #dee2e6; background-color: #f8f9fa; font-weight: 600;">Net</th></tr></thead><tbody>' + department.transactions.map(function(t){ return '<tr><td style="text-align: left; padding: 8px; border: 1px solid #dee2e6;">' + (typeof formatDate==='function' ? formatDate(t.date) : new Date(t.date).toLocaleDateString()) + '</td><td style="text-align: left; padding: 8px; border: 1px solid #dee2e6;">' + t.branch + '</td><td style="text-align: left; padding: 8px; border: 1px solid #dee2e6;">' + t.subDepartment + '</td><td style="text-align: right; padding: 8px; border: 1px solid #dee2e6;">' + t.grossSale.toLocaleString() + '</td><td style="text-align: right; padding: 8px; border: 1px solid #dee2e6;">' + t.discount.toLocaleString() + '</td><td style="text-align: right; padding: 8px; border: 1px solid #dee2e6;">' + t.returnAmount.toLocaleString() + '</td><td style="text-align: right; padding: 8px; border: 1px solid #dee2e6;">' + t.gst.toLocaleString() + '</td><td style="text-align: right; padding: 8px; border: 1px solid #dee2e6;">' + ((t.cost||0).toLocaleString()) + '</td><td style="text-align: right; padding: 8px; border: 1px solid #dee2e6;"><strong>' + t.netSale.toLocaleString() + '</strong></td></tr>'; }).join('') + '<tr class="table-warning fw-bold" style="background-color: #fff3cd;"><td colspan="3" style="text-align: right; padding: 8px; border: 1px solid #dee2e6;"><strong>Department Total</strong></td><td style="text-align: right; padding: 8px; border: 1px solid #dee2e6;"><strong>' + department.grossSale.toLocaleString() + '</strong></td><td style="text-align: right; padding: 8px; border: 1px solid #dee2e6;"><strong>' + department.discount.toLocaleString() + '</strong></td><td style="text-align: right; padding: 8px; border: 1px solid #dee2e6;"><strong>' + department.returnAmount.toLocaleString() + '</strong></td><td style="text-align: right; padding: 8px; border: 1px solid #dee2e6;"><strong>' + department.gst.toLocaleString() + '</strong></td><td style="text-align: right; padding: 8px; border: 1px solid #dee2e6;"><strong>' + ((department.cost||0).toLocaleString()) + '</strong></td><td style="text-align: right; padding: 8px; border: 1px solid #dee2e6;" class="text-success"><strong>' + department.netSale.toLocaleString() + '</strong></td></tr></tbody></table></div></div></div>'; summaryCards.appendChild(card); });
  }

  function loadDeptSalesList(page, perPage){ page = page || 1; perPage = perPage || 10; var dateFrom = document.getElementById('deptSalesListDateFrom') && document.getElementById('deptSalesListDateFrom').value || ''; var dateTo = document.getElementById('deptSalesListDateTo') && document.getElementById('deptSalesListDateTo').value || ''; var branchId = document.getElementById('deptSalesListBranch') && document.getElementById('deptSalesListBranch').value || ''; var departmentId = document.getElementById('deptSalesListDepartment') && document.getElementById('deptSalesListDepartment').value || ''; if (!dateFrom && !dateTo){ var invoicesContainer = document.getElementById('deptSalesInvoicesContainer'); var salesTableBody = document.getElementById('deptSalesListTableBody'); if (invoicesContainer) invoicesContainer.innerHTML = '<div class="text-center text-muted p-4">Please select at least one date (From or To) to view department sales list</div>'; if (salesTableBody) salesTableBody.innerHTML = '<tr><td colspan="11" class="text-center text-muted">Please select at least one date (From or To) to view department sales list</td></tr>'; var invoicesPagination = document.getElementById('deptSalesInvoicesPagination'); var listPagination = document.getElementById('deptSalesListPagination'); if (invoicesPagination) invoicesPagination.innerHTML=''; if (listPagination) listPagination.innerHTML=''; if (typeof showNotification === 'function') showNotification('Please select at least one date (From or To) to view department sales list', 'info'); return; } var filters = {}; if (dateFrom) filters.from = dateFrom; if (dateTo) filters.to = dateTo; if (branchId) filters.branchId = branchId; if (departmentId) filters.departmentId = departmentId; api.getDepartmentSales(filters).then(function(salesData){ var sortedSales = salesData.slice().sort(function(a,b){ return new Date(b.date) - new Date(a.date); }); var grandTotals = { grossSale:0, discountAmount:0, netSale:0, returnAmount:0, gst:0, cost:0 }; salesData.forEach(function(sale){ grandTotals.grossSale += sale.grossSale || 0; grandTotals.discountAmount += sale.discountAmount || 0; grandTotals.netSale += sale.netSale || 0; grandTotals.returnAmount += sale.returnAmount || 0; grandTotals.gst += sale.gst || 0; grandTotals.cost += sale.cost || 0; }); loadDepartmentSummary(salesData); var startIndex = (page - 1) * perPage; var endIndex = startIndex + perPage; var paginatedSales = sortedSales.slice(startIndex, endIndex); loadDeptSalesInvoices(paginatedSales); loadDeptSalesTable(paginatedSales, grandTotals); updatePagination(sortedSales.length, page, perPage, 'deptSalesInvoicesPagination', function(newPage, pp){ loadDeptSalesList(newPage, pp); }); updatePagination(sortedSales.length, page, perPage, 'deptSalesListPagination', function(newPage, pp){ loadDeptSalesList(newPage, pp); }); }).catch(function(error){ console.error('Error loading department sales list:', error); if (typeof showNotification === 'function') showNotification('Failed to load department sales list', 'error'); }); }
